...
double swmm_loadetmatric () {
    ETMatric[ET_ROW][BASINSNUM];
    int row = 0;
    int col = 0;
    char *line_buf = NULL;
    size_t line_buf_size = 0;
    FILE *fp;
    fp = fopen ("bigm.csv", "r");      //open file , read only
    if (!fp) {
        printf (stderr, "failed to open file for reading");
        return 1;
    }
  ...
  {
    double *ptr = strtok(line_buf, ";");
    
   	while(ptr != NULL)
	{
        ETMatric[row][col] = (double)atof(ptr);
        //printf("Evap.rate is %f",ETMatric[row][col]);
        //printf("\n");
		ptr = strtok(NULL, ";");
        col++;
...
//load normalized et matrix
//user defined

double swmm_loadetmatricn () {
    ETMatricn[ET_ROW][BASINSNUM];
    int row = 0;
    int col = 0;
    char *line_buf = NULL;
    size_t line_buf_size = 0;
    FILE *fp;
    fp = fopen ("bigmn.csv", "r");      //open file , read only
    if (!fp) {
        printf (stderr, "failed to open file for reading");
        return 1;
    }
  ...
  while (getline(&line_buf, &line_buf_size, fp) >= 0)
  {
    double *ptr = strtok(line_buf, ";");
   	...
   	while(ptr != NULL)
	{
        ETMatricn[row][col] = (double)atof(ptr);
        //printf("Evap.rate is %f",ETMatric[row][col]);
        //printf("\n");
		ptr = strtok(NULL, ";");
        col++;
