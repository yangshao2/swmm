...
static double getEvapRate(int j);
//get soil recovery matrix
static double getEvapRaten(int j);
...
//
{
    ...
    double evapRaten  = 0.0;            // evaporation rate normalized for soil recovery
    ...
    //else evapRate = getEvapRate(j);//Evap.rate;
    
    evapRate = getEvapRate(j);//Evap.rate;
    evapRaten = getEvapRaten(j);//Evap.rate normalized;
...
double getEvapRate(int j){
    
    int day = floor(ElapsedTime);
    double newEvaprate=ETMatric[day][j];
    //Evap.rate = ETMatric[day][j];
    


    Evap.rate = newEvaprate/26334719.16;
    
    //printf("j is %i, Evap.rate is %f",j,Evap.rate);
    //printf("\n");
    //if (j == 27){
...
double getEvapRaten(int j){
    
    int day = floor(ElapsedTime);
    double newEvapraten=ETMatricn[day][j];
    
    Evap.recoveryFactor = newEvapraten;


    return Evap.recoveryFactor;
}
...
//
{
 ...
 //here we try to print change of surfMoisture 
 //  printf("%f",subarea->depth);
 //   printf("\n");
